...
import CustomerPlaceOrder from "./CustomerPlaceOrder";
...
const navItems: Array<{
  ...
  { id: "place-order", label: "Place New Order", icon: ShoppingCart },
  { id: "completed-orders", label: "My Completed Orders", icon: ClipboardList },
...
export default function CustomerLayout({ customer, onLogout }: Props) {
    ...
    <div className="flex h-screen overflow-hidden bg-background">
          ...
          {navItems.map((item) => {
            ...
            return (
              <button
                type="button"
                key={item.id}
                data-ocid={`customer.nav.${item.id}.button`}
      ...
      <div className="flex-1 flex flex-col min-w-0 overflow-hidden">
        ...
        <main className="flex-1 overflow-y-auto bg-background">
          {section === "dashboard" && (
            <CustomerDashboard customer={customer} onNavigate={navigate} />
          )}
          {section === "place-order" && (
            <CustomerPlaceOrder
              customer={customer}
              onOrderPlaced={() => navigate("dashboard")}
            />
          )}
          {section === "completed-orders" && (
            <CustomerCompletedOrders customer={customer} />
          )}
          {section === "statement" && <CustomerStatement customer={customer} />}
        </main>
      </div>
    </div>
  );
